...
def test_event_ids_unique_and_bound_to_original_scene():
    ids={s['id'] for s in M['scenes']};assert len(E['events'])==26;assert len({x['event_id'] for x in E['events']})==len(E['events']);assert all(e['scene_id'] in ids for e in E['events'])
def test_solo_npc_events_are_not_player_qualifiers():
    npc=[e for e in E['events'] if e['record_owner']!='PLAYER']
    assert {e['event_id'] for e in npc}=={'r01_ian_separate_local','r02_ian_river_broadcast','r04_narae_qualifier','r04_doyun_ian_invite','r06_eunsol_independent','r08_doyun_wildcard'}
    assert next(e for e in npc if e['event_id']=='r01_ian_separate_local')['npc_card_action_replay_status']=='ENGINE_REPLAY'
    assert all(e['npc_card_action_replay_status']=='NOT_IMPLEMENTED' for e in npc if e['event_id']!='r01_ian_separate_local')
...
def test_extra_npc_matches_from_the_original_script_do_not_force_player_record():
    ...
    assert all(e['npc_card_action_replay_status']=='NOT_IMPLEMENTED' for id,e in new.items() if id!='r01_ian_separate_local')
