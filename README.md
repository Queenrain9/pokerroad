...
- Production runtime bindings authored against canonical source: **18/88**. The first sixteen main scenes (`01-M01`–`01-M07`, `02-M01`–`02-M06`, `03-M01`–`03-M03`) now have physical targets, choices, and completion paths; `05-M03` and `08-M03` remain semantic bindings only.
...
- Playable implemented story scenes: **16/88**. The first sixteen now have source-backed reach/interact/action/state/exit flows; the second batch adds Han River broadcast/boarding/boat play and Forest vertical-route/showcase play. Final art and device layout are separate work.
- GitHub Core QA runs structural/source/data tests on normal pushes. The exact **2,598,960** five-card exhaustive classification remains available as an explicit manual milestone check instead of running on every content push. See `docs/P5_M01_M08_PLAYABLE.md` and `docs/P5_M09_M16_PLAYABLE.md`.
- Godot native CI keeps the first-eight regression and now also runs dedicated M09–M16 physical-story and poker/save E2E suites. The latest CI result is the executable verification authority; Windows device play remains separate.
