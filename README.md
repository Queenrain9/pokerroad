...
- Production runtime bindings authored against canonical source: **33/88**. The first thirty-two main scenes (`01-M01`–`01-M07`, `02-M01`–`02-M06`, `03-M01`–`03-M05`, `04-M01`–`04-M06`, `05-M01`–`05-M05`, `06-M01`–`06-M03`) now have physical targets, choices, and completion paths; `08-M03` remains a semantic binding only.
...
- Playable implemented story scenes: **32/88**. The latest batch carries the mainline through all five Last Station main scenes and the first three Palace main scenes, including Narae's self-filed objection, explicit train/B-line/free-bus routes, Doyun's provisional reservation reveal and optional direct match, Palace public entry, Eunsol's independent NPC replay, and Wolin's free tower-lobby visit. Final art and device layout are separate work.
- GitHub Core QA runs structural/source/data tests on normal pushes. The exact **2,598,960** five-card exhaustive classification remains available as an explicit manual milestone check instead of running on every content push. See `docs/P5_M01_M08_PLAYABLE.md`, `docs/P5_M09_M16_PLAYABLE.md`, `docs/P5_M17_M24_PLAYABLE.md`, and `docs/P5_M25_M32_PLAYABLE.md`.
- Godot native CI keeps the earlier regressions and now also runs dedicated M09–M16, M17–M24, and M25–M32 physical-story plus poker/save/qualification/route E2E suites. The latest CI result is the executable verification authority; Windows device play remains separate.
