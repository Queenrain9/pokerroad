...
- Production runtime bindings authored against canonical source: **26/88**. The first twenty-four main scenes (`01-M01`–`01-M07`, `02-M01`–`02-M06`, `03-M01`–`03-M05`, `04-M01`–`04-M06`) now have physical targets, choices, and completion paths; `05-M03` and `08-M03` remain semantic bindings only.
...
- Playable implemented story scenes: **24/88**. The latest batch carries the mainline through the rest of Forest and all six Harbor main scenes, including tide-safe rescue traversal, Narae's independent qualifier, the player official qualifier review seam, Doyun vs Ian, and regular rail departure to Region 05. Final art and device layout are separate work.
- GitHub Core QA runs structural/source/data tests on normal pushes. The exact **2,598,960** five-card exhaustive classification remains available as an explicit manual milestone check instead of running on every content push. See `docs/P5_M01_M08_PLAYABLE.md`, `docs/P5_M09_M16_PLAYABLE.md`, and `docs/P5_M17_M24_PLAYABLE.md`.
- Godot native CI keeps the earlier regressions and now also runs dedicated M09–M16 and M17–M24 physical-story plus poker/save/qualification E2E suites. The latest CI result is the executable verification authority; Windows device play remains separate.
