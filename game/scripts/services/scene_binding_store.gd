...
static func validate_catalog() -> Array[String]:
	...
	var seen := {}
	for binding in data.get("scenes", []):
		var scene_id := str(binding.get("scene_id", ""))
		...
		var source_path := "res://data/original_scenes/" + scene_id + ".json"
		...
		var region_id := str(binding.get("region_id", ""))
		...
		var open_mode := str(binding.get("open_mode", ""))
		for interaction in binding.get("interactions", []):
			for choice in interaction.get("choices", []):
				var requested_mode := str(choice.get("requested_mode", open_mode))
